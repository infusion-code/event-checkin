...
export class CurrentNavService extends CurrentNavProvider {
    ...
    protected GetData() {
        ...
        if(this._config.IsAuthenticated){ 
            ...
            this._subscription = events.subscribe(x => {
                ...
                // register nav propvider with user service so that post authentication navigation can happen from here on out
                ...
                if(this._userService.RequestedPostAuthenticationRedirect != null && this._userService.RequestedPostAuthenticationRedirect !== "") {
                    Observable.timer(1).subscribe(e => {
                        this._routeRequest.next(<string>this._userService.RequestedPostAuthenticationRedirect);
                    });
    ...
    public Route(url:string){
        if(url && url != "") this._router.navigateByUrl(url);
        else if(this._root.Children && this._root.Children.length > 0){
            this._router.navigateByUrl(this._root.Children[0].Url);
        }
        else{
